...
- ✅ 2026-05-24: Refactored index.html into modular file structure. index.html is now a shell only. CSS split into css/{base,layout,components,builder,play}.css. JS split into 17 ES modules across js/shared/, js/data/, js/builder/, js/play/, and js/main.js. All localStorage centralized in storage.js, all fetch in loader.js, all overlays in overlay.js. Window globals wired in main.js for onclick compatibility. Merges PHB/XGE/TCE, filters level 0/1 by class, deduped + sorted alpha. Each spell shows school, cast time, range, first sentence. Falls back to hardcoded on cache miss. Stores selection in `localStorage('oneshot_ruleset')`, switching clears character data. Fetches races.json, 12 class files, 3 spell files in parallel via Promise.all; results cached in `DATA_CACHE` keyed by edition. Phase 1 hidden until fetch resolves. Builder steps still use hardcoded fallback data — wiring live data is next.
