...
- ✅ 2026-05-24: Wired race and class builder steps to DATA_CACHE via `getActiveRaces()` / `getActiveClasses()` helpers. Falls back to hardcoded RACES/CLASSES on cache miss. Hit die, saves, and skill list/count pulled live from class JSON; race bonuses and speed pulled live from races.json. Merges PHB/XGE/TCE, filters level 0/1 by class, deduped + sorted alpha. Each spell shows school, cast time, range, first sentence. Falls back to hardcoded on cache miss. Stores selection in `localStorage('oneshot_ruleset')`, switching clears character data. Fetches races.json, 12 class files, 3 spell files in parallel via Promise.all; results cached in `DATA_CACHE` keyed by edition. Phase 1 hidden until fetch resolves. Builder steps still use hardcoded fallback data — wiring live data is next.
